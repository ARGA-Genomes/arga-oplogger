...
const ASSEMBLY_SUMMARY_URL: &'static str = "https://ftp.ncbi.nlm.nih.gov/genomes/refseq/assembly_summary_refseq.txt";
const ASSEMBLY_SUMMARY_FILE: &'static str = "assembly_summary_refseq.txt";
...
pub fn package(meta: Meta) -> Result<String, ExtractError> {
    let filename = format!("ncbi-refseq-{}.tar", meta.dataset.published_at.to_string());
...
pub fn meta(version: &str, published_at: &str) -> Result<Meta, ExtractError> {
    ...
    let dataset = Dataset {
        id: "".into(),
        name: "NCBI: Refseq Nucleotide".into(),
        short_name: "NCBI: GCF".into(),
        version: version.into(),
        published_at,
        url: "https://www.ncbi.nlm.nih.gov/refseq/".into(),
